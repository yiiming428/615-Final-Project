...
longer_data <- filter_data |> 
  pivot_longer(cols = `1973`:`2022`, names_to = "Year", values_to = "Value") |> 
  mutate(
    Year = as.numeric(Year), 
    Value = as.numeric(gsub("[^0-9.-]", "", Value))
  )
...
#addResourcePath("images", '/Users/cc./Desktop/615 Final Project/615FinalProject/www')
             ...
             tabPanel("Introduction",
                      fluidRow(
                        ...
                        column(6, align = "center",  # Add the image on the right-hand side
                               img(src = "WechatIMG20.jpg", height = "500px", width = "90%", alt = "Indonesia Overview")
                        )
                      ),
                      fluidRow(
                        column(6, wellPanel(
